...
export class PaymentsService {
  ...
  async createCharge({ card, amount, email }: PaymentCreateChargeDto){
    ...
    const createPaymentIntent = await this.stripe.paymentIntents.create({
    ...
    this.notificationService.emit('notify_email', { email, text: `Your Payment Reservation of $ ${ amount  } has completed successfully` })
